        ...
        <div class="col-md-4">
            <div class="input-group">
                <input type="text" id="searchInput" class="form-control" placeholder="Enter phone number..."
                    autocomplete="off">
                <button type="button" id="clearSearch" class="btn btn-success">
                    Clear
                </button>
            </div>
                    ...
                    <?php foreach ($rows as $b): ?>
                        <tr class="booking-row">
                            <form method="POST">
                                ...
                                <td><?= date('M j', strtotime($b['check_in'])) ?> <strong>To</strong>
                                    <?= date('M j', strtotime($b['check_out'])) ?></td>
                                ...
                                <td>
                                    <select name="status" class="form-select form-select-sm">
                                        ...
                                        <option value="approved" <?= $b['status'] == 'approved' ? 'selected' : '' ?>>Approved
                                        </option>
                                        <option value="rejected" <?= $b['status'] == 'rejected' ? 'selected' : '' ?>>Rejected
                                        </option>
                                ...
                                <!-- Payment -->
                                <td>
                                    <?php $payment = !empty($b['payment_status']) ? $b['payment_status'] : 'checking'; ?>
                                    <select name="payment_status" class="form-select form-select-sm">
                                        <option value="checking" <?= $payment == 'checking' ? 'selected' : '' ?>>Checking</option>
                                        <option value="paid" <?= $payment == 'paid' ? 'selected' : '' ?>>Paid</option>
                                        <option value="20%" <?= $payment == '20%' ? 'selected' : '' ?>>20% Paid</option>
                                    </select>
                                    <span class="badge 
<?= $payment == 'paid' ? 'bg-success' : ($payment == '20%' ? 'bg-primary' : 'bg-warning') ?> text-white"
                                        style="display: block; margin-top: 5px;">
                                        <?= $payment == '20%' ? '20% Paid' : ucfirst($payment) ?>
                                    </span>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-info btn-sm mb-1 w-100 view-details"
                                        data-id="<?= $b['id'] ?>" data-room="<?= htmlspecialchars($b['room_name']) ?>"
                                    ...
                                    <?php else: ?>
                                        <button type="submit" name="send_invoice" class="btn btn-success btn-sm w-100">Send
                                            Invoice</button>
                                    <?php endif; ?>
                                </td>
                            </form>
